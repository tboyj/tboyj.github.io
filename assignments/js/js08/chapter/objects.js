...
let pokerGame = {
   ...
   placeBet: function (type) {
      ...
      this.currentBank += this.currentBet * pay;
...
//constructor function for a poker deck
function pokerDeck() {
   this.cards = [];
   ...
   for (let s = 0; s < suits.length; s++) {
      for (let r = 0; r < ranks.length; r++) {
         this.cards.push(new pokerCard(ranks[r], suits[s]));
      }
   }
   ...
   pokerHand.cards = this.cards.splice(0, pokerHand.handLength);
};
}
